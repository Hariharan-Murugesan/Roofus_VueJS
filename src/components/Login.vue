      ...
      <div class="card-container w-100">
        <div class="card px-8 py-8 border-0 shadow rounded-lg" id="form1">
          <div class="form-data" v-if="!submitted">
            <div class="title text-center"><h2>LOGIN</h2></div>
            ...
            <router-link
              to="/register"
              class="text-right d-block mt-2 primary-clr"
    ...
    async submit() {
      ...
      if (this.valid) {
        ...
        try {
          ...
          if (op.status === 201) {
            localStorage.setItem("userId", op.data.response[0]._id);
            localStorage.setItem("userName", op.data.response[0].name);
            this.$router.push({ path: "/home" });
          }
        } catch (error) {
